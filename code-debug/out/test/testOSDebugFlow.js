...
class MockMI2 {
    ...
    async getStack(_start, _max, _thread) {
        return this.stackResponse;
    }
    async getSomeRegisterValues(_ids) {
    ...
    addSymbolFile(filepath, _textAddr) {
...
class OSDebugHarness {
    constructor() {
        this.events = [];
        this.kernelMemoryRanges = [['0xffffffc000000000', '0xffffffffffffffff']];
        this.userMemoryRanges = [['0x0000000000000000', '0x0000004000000000']];
        this.programCounterId = 32;
        ...
        const self = this;
        const bpgSession = {
            get miDebugger() { return self.mockMI2; },
            ...
            onBreakpointsRestored(_results) { },
        };
        this.breakpointGroups = new breakpointGroups_1.BreakpointGroups('kernel', bpgSession, 'user');
    }
    ...
    doAction(action) {
        if (action.type === OSStateMachine_1.DebuggerActions.check_stop_in_kernel) {
        ...
        else if (action.type === OSStateMachine_1.DebuggerActions.check_if_kernel_yet) {
            this.mockMI2.getSomeRegisterValues([this.programCounterId]).then(regs => {
                if (!regs || regs.length === 0 || !regs[0])
                    return;
                const pc = (0, addrSpace_1.parseAddr)(regs[0].value ?? '');
...
async function runTests() {
    // --- Scenario 1: kernel state, user breakpoint hit (not border, not hook) ---
    {
        ...
        h.osStateTransition(new OSStateMachine_1.OSEvent(OSStateMachine_1.OSEvents.STOPPED));
        ...
        assertEq(h.osState.status, OSStateMachine_1.OSStates.kernel, 'S1: stays in kernel state');
    }
    // --- Scenario 2: kernel state, hook breakpoint hit ---
    {
        ...
        assertEq(h.osState.status, OSStateMachine_1.OSStates.kernel, 'S2: stays in kernel state');
    }
    // --- Scenario 3: kernel state, border breakpoint hit ---
    {
        const h = new OSDebugHarness();
        // Register a border at /src/trap.rs:200
        h.breakpointGroups.updateBorder(new breakpointGroups_1.Border('/src/trap.rs', 200));
        ...
        assertEq(h.osState.status, OSStateMachine_1.OSStates.kernel_single_step_to_user, 'S3: state → kernel_single_step_to_user');
    }
    // --- Scenario 4: single-stepping, PC still in kernel ---
    {
        const h = new OSDebugHarness();
        h.osState = new OSStateMachine_1.OSState(OSStateMachine_1.OSStates.kernel_single_step_to_user);
        ...
        assertEq(h.osState.status, OSStateMachine_1.OSStates.kernel_single_step_to_user, 'S4: stays in kernel_single_step_to_user');
    ...
    // --- Scenario 5: single-stepping, PC arrives at user space ---
    {
        ...
        h.osStateTransition(new OSStateMachine_1.OSEvent(OSStateMachine_1.OSEvents.STOPPED));
        await new Promise(r => setTimeout(r, 10));
        assertEq(h.osState.status, OSStateMachine_1.OSStates.user, 'S5: state → user');
    ...
    {
        const h = new OSDebugHarness();
        h.osState = new OSStateMachine_1.OSState(OSStateMachine_1.OSStates.user);
        ...
        assertEq(h.osState.status, OSStateMachine_1.OSStates.user, 'S6: stays in user state');
    }
    // --- Scenario 7: user state, border breakpoint hit (user→kernel) ---
    {
        ...
        h.breakpointGroups.updateBorder(new breakpointGroups_1.Border('/app/src/syscall.rs', 50));
        ...
        assertEq(h.osState.status, OSStateMachine_1.OSStates.user_single_step_to_kernel, 'S7: state → user_single_step_to_kernel');
    }
    // --- Scenario 8: full cycle kernel → user → kernel ---
    {
        ...
        h.osStateTransition(new OSStateMachine_1.OSEvent(OSStateMachine_1.OSEvents.STOPPED));
        await new Promise(r => setTimeout(r, 10));
        assertEq(h.osState.status, OSStateMachine_1.OSStates.kernel_single_step_to_user, 'S8a: entered single-step to user');
        // Step 2: single-step, still in kernel
        h.mockMI2.registerResponse = [{ index: 32, value: '0xffffffc0000a0004' }];
        h.osStateTransition(new OSStateMachine_1.OSEvent(OSStateMachine_1.OSEvents.STOPPED));
        await new Promise(r => setTimeout(r, 10));
        assertEq(h.osState.status, OSStateMachine_1.OSStates.kernel_single_step_to_user, 'S8b: still stepping');
        // Step 3: single-step, arrived at user
        h.mockMI2.registerResponse = [{ index: 32, value: '0x0000000010000000' }];
        h.osStateTransition(new OSStateMachine_1.OSEvent(OSStateMachine_1.OSEvents.STOPPED));
        await new Promise(r => setTimeout(r, 10));
        assertEq(h.osState.status, OSStateMachine_1.OSStates.user, 'S8c: arrived at user');
        // Step 4: user state, register a user→kernel border
        h.breakpointGroups.updateBorder(new breakpointGroups_1.Border('/app/src/syscall.rs', 50));
        h.mockMI2.stackResponse = [{ level: 0, address: '0x0000000010002000', function: 'ecall', fileName: 'syscall.rs', file: '/app/src/syscall.rs', line: 50 }];
        h.osStateTransition(new OSStateMachine_1.OSEvent(OSStateMachine_1.OSEvents.STOPPED));
        await new Promise(r => setTimeout(r, 10));
        assertEq(h.osState.status, OSStateMachine_1.OSStates.user_single_step_to_kernel, 'S8d: entered single-step to kernel');
        // Step 5: single-step, arrived at kernel
        h.mockMI2.registerResponse = [{ index: 32, value: '0xffffffc000080000' }];
        h.osStateTransition(new OSStateMachine_1.OSEvent(OSStateMachine_1.OSEvents.STOPPED));
        await new Promise(r => setTimeout(r, 10));
        assertEq(h.osState.status, OSStateMachine_1.OSStates.kernel, 'S8e: back to kernel');
...
//# sourceMappingURL=testOSDebugFlow.js.map