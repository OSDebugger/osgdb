...
export class GDBDebugSession extends DebugSession {
        ...
        if (config.border_breakpoints) {
            for (const b of config.border_breakpoints) {
                ...
                if ('marker' in b) {
                    ...
                    if (found.length === 0) {
                        this.sendEvent(new OutputEvent(`[osdb] Warning: marker "${b.marker}" not found in ${this.cwd}\n`, 'stderr'));
                    }
        ...
        if (config.hook_breakpoints) {
            for (const h of config.hook_breakpoints) {
                ...
                if ('marker' in h) {
                    ...
                    if (found.length === 0) {
                        this.sendEvent(new OutputEvent(`[osdb] Warning: marker "${h.marker}" not found in ${this.cwd}\n`, 'stderr'));
                    }
                    for (const loc of found) {
    ...
    private async fallbackPhysicalStackTrace(
        ...
        const stackFrames: DebugProtocol.StackFrame[] = stack.map((f, i) => {
            ...
            let privilegeTag = '';
            if (f.address) {
                const addr = parseAddr(f.address);
                if (addr !== undefined) {
            ...
            const sf = new StackFrame(
                frameId,
                (f.function || '<unknown>') + privilegeTag,
